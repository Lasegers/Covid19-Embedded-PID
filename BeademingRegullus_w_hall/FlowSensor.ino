#include <Wire.h>
#include "sdpsensor.h"
volatile bool IS_FLOW_SENSOR_INITIALIZED = false;
//----------------------------------------------------------------------------------------------------------------
// SDP3x on the default I2C address of 0x21:
volatile SDP3XSensor sdp;
//lookup table for forward current (mA) vs. normalized radiant flux. Key-value pair: {current, flux}
volatile const double DP_vs_SLM[][2] = {{0.0  ,0.0},{0.15 ,1.1},{0.32 ,2.1},{0.54 ,3.2},{0.79 ,4.3},
...
//----------------------------------------------------------------------------------------------------------------
volatile char SLM[10];
//----------------------------------------------------------------------------------------------------------------
bool FLOW_SENSOR_INIT()
{
  if (TWCR == 0)  Wire.begin();  // start wire only if not started yet 
